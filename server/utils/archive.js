 ...
 */
export default function zipDir(inputPath, outputPath, logger) {
    return new Promise((resolve, reject) => {
        const output = fs.createWriteStream(outputPath);
        const archive = archiver("zip", { zlib: { level: 9 } });

        output.on("close", () => {
            logger?.info(`ZIP Archiv erstellt: ${archive.pointer()} Bytes`);
            resolve();
        });

        output.on("error", (err) => {
            logger?.error("Error beim WriteStream in zipDir", { error: err });
            reject(err);
        });

        archive.on("warning", (err) => {
            if (err.code !== "ENOENT") {
                logger?.error("Warning beim Archivieren", { error: err });
                reject(err);
            }
        });

        archive.on("error", (err) => {
            logger?.error("Error beim Archivieren", { error: err });
            reject(err);
        });
